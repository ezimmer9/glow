...
void metadata::get_size(std::map<int , std::shared_ptr<int>> size){
...
void metadata::write_to_file(std::ofstream& output){
...
void metadata::read_from_file(std::ifstream& file){
	unsigned int offset;
...
void En2grCommon::loadMatrixFromFile(llvm::StringRef filename, Tensor &result) {
...
/// Loads tensor of floats from binary file.
void En2grCommon::loadMatrixAndTransposeFromFile(llvm::StringRef filename, Tensor &result) {
...
/// Loads tensor of floats from binary file.
void En2grCommon::loadMatrixAndSplitFromFile(llvm::StringRef filename, std::vector<Constant *> result, uint numOfSlices) {
	std::ifstream file(filename.str(), std::ios::binary);
	assert(file.is_open() == true);
	metadata meta;
	meta.read_from_file(file);
...
void En2grCommon::loadMatrixAndSplitAndTransposeFromFile(llvm::StringRef filename, std::vector<Constant *> result, uint numOfSlices) {
	std::ifstream file(filename.str(), std::ios::binary);
	assert(file.is_open() == true);
	metadata meta;
	meta.read_from_file(file);
...
void En2grCommon::print_weights(std::vector<Constant *> weights){
	for (uint k = 0 ; k < 4 ; k++){
		std::cout << weights[k]->getName().str() << "\n";
		for (uint i = 0; i < weights[k]->dims()[0] ; i+=32){
			for (uint j = 0; j < weights[k]->dims()[1] ; j+=32){
				std::cout << "i: " << i << " j: " << j << "  " << weights[k]->getHandle<float_t>().at({i,j}) <<  "\n";
			}
		}
	}
}
...
void En2grCommon::init_hidden(std::vector<std::vector<Node *>> &hidden , uint length , Node *node){
